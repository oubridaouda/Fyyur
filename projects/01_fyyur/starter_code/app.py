...
def show_venue(venue_id):
  ...
  arrives = Artist.query.join(Show)
  data1={
    "id": venues.id,
    "name": venues.name,
    ...
    "past_shows": [{
      "artist_id": artists.filter_by(id=4).first().id,
      "artist_name": artists.filter_by(id=4).first().name,
      "artist_image_link": artists.filter_by(id=4).first().image_link,
...
def create_artist_submission():
  ...
  try:
    ...
    seeking_talent = request.form.get('seeking_talent','')
    seeking_description = request.form.get('seeking_description','')
    artist = Artist(name=name,city=city,state=state,phone=phone,genres=genres,image_link=image_link,facebook_link=facebook_link,website_link=website_link,seeking_talent=seeking_talent,seeking_description=seeking_description)
    db.session.add(artist)
    db.session.commit()
    # on successful db insert, flash success
